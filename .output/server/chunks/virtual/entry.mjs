...
import { getCurrentScope, ref, watchEffect, getCurrentInstance, onBeforeUnmount, onDeactivated, onActivated, createApp, provide, onErrorCaptured, onServerPrefetch, unref, createVNode, resolveDynamicComponent, shallowReactive, reactive, effectScope, hasInjectionContext, inject, defineAsyncComponent, mergeProps, defineComponent, computed, toRef, h, isReadonly, useSSRContext, isRef, isShallow, isReactive, toRaw } from 'vue';
...
import { ssrRenderSuspense, ssrRenderComponent, ssrRenderVNode, ssrRenderAttrs, ssrRenderAttr, ssrInterpolate, ssrRenderClass, ssrRenderStyle, ssrRenderList, ssrIncludeBooleanAttr } from 'vue/server-renderer';
...
var app_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	setup(__props) {
		...
		const role = ref("host"), screen = ref("lobby"), qr = ref(""), name = ref("You"), ready = ref(false), taps = ref(0), seconds = ref(25);
		...
		const allRacers = computed(() => [...racers, {
			name: name.value,
			color: "#49a96e",
			score: growth.value
		}].sort((a, b) => b.score - a.score));
		return (_ctx, _push, _parent, _attrs) => {
			_push(`<main${ssrRenderAttrs(mergeProps({ class: "shell" }, _attrs))}><nav><a class="brand" href="#"><span>✿</span> BLOOM RUSH</a><div class="live"><i></i> ROOM #SPROUT</div></nav>`);
			if (unref(screen) === "lobby") {
				_push(`<section class="lobby"><div class="intro"><div class="eyebrow">HOST CONTROL</div><h1>Scan. Join.<br><em>Grow!</em></h1><p>Ask every gardener to scan the code. They’ll wait safely in the lobby until you start the race.</p><div class="steps"><span><b>1</b> Scan QR</span><span><b>2</b> Wait in lobby</span><span><b>3</b> Host starts</span></div></div><div class="join-card host-card"><div class="card-top"><span>SCAN TO JOIN</span><strong>#SPROUT</strong></div>`);
				if (unref(qr)) _push(`<img class="real-qr"${ssrRenderAttr("src", unref(qr))} alt="QR code to join">`);
				else _push(`<div class="qr-loading">Growing QR…</div>`);
				_push(`<h2>Join the watering race</h2><p>Participants scan this code and wait for you.</p><div class="ready-list"><span><i>M</i>Maya <b>READY</b></span><span><i>L</i>Leo <b>READY</b></span><span><i>N</i>Nina <b>READY</b></span></div><button>START GAME <span>→</span></button><small><i></i> 3 gardeners waiting</small></div></section>`);
			} else if (unref(screen) === "waiting") {
				_push(`<section class="waiting-page"><div class="waiting-card"><div class="seed-pulse">🌱</div><div class="eyebrow">ROOM #SPROUT</div><h1>You’re in!</h1><p>${ssrInterpolate(unref(ready) ? "You’re ready to grow. The game will begin when the host presses Start Game." : "Choose your gardener name, then get ready.")}</p>`);
				if (!unref(ready)) _push(`<!--[--><label>Your nickname<input${ssrRenderAttr("value", unref(name))} maxlength="12"></label><button>I’M READY <span>→</span></button><!--]-->`);
				else _push(`<div class="waiting-status"><i></i><b>WAITING FOR HOST</b><span>Keep this page open</span></div>`);
				_push(`<div class="waiting-dots"><i></i><i></i><i></i></div></div></section>`);
			} else {
				_push(`<section class="game"><header class="game-head"><div><button class="back">←</button><span>${ssrInterpolate(unref(role) === "host" ? "HOST VIEW" : "ROOM #SPROUT")}</span></div><div class="timer"><small>TIME LEFT</small><b>00:${ssrInterpolate(String(unref(seconds)).padStart(2, "0"))}</b></div><div class="rank"><small>YOUR RANK</small><b>#${ssrInterpolate(unref(rank))}</b></div></header><div class="race-grid"><div class="garden-panel"><div class="water-drops">💧　💧</div><div class="plant-wrap"><div class="sun">☀</div><div class="${ssrRenderClass([{ "finished-flower": unref(finished) }, "plant"])}" style="${ssrRenderStyle({ transform: `scale(${.7 + unref(growth) / 260})` })}">${ssrInterpolate(unref(stage))}</div><div class="soil"></div></div><div class="growth-copy"><span>YOUR PLANT</span><b>FULL BLOOM GOAL</b></div><div class="meter"><i style="${ssrRenderStyle({ width: `${unref(growth)}%` })}"></i></div></div><div class="leaderboard"><div class="eyebrow">LIVE LEADERBOARD</div><h2>Garden race</h2><!--[-->`);
				ssrRenderList(unref(allRacers), (r, i) => {
					_push(`<div class="${ssrRenderClass([{ you: r.name === unref(name) }, "racer"])}"><strong>${ssrInterpolate(i + 1)}</strong><i style="${ssrRenderStyle({ background: r.color })}">${ssrInterpolate(r.name[0]?.toUpperCase())}</i><span>${ssrInterpolate(r.name)}<small>${ssrInterpolate(r.score >= 85 ? "Blooming!" : r.score >= 60 ? "Growing strong" : "Keep tapping")}</small></span><b>${ssrInterpolate(r.score)}%</b></div>`);
				});
				_push(`<!--]--></div></div><div class="tap-zone"><div><span>${ssrInterpolate(unref(finished) ? "🏆" : unref(seconds) <= 0 ? "⏱" : "💧")}</span><h2>${ssrInterpolate(unref(finished) ? "You bloomed!" : unref(seconds) <= 0 ? "Time’s up!" : "Water your plant!")}</h2><p>${ssrInterpolate(unref(finished) ? "Your flower reached a glorious full bloom!" : unref(seconds) <= 0 ? "The garden needs a little more water." : "Keep tapping to grow a full bloom")}</p></div><button class="water-btn"${ssrIncludeBooleanAttr(unref(seconds) <= 0 || unref(finished)) ? " disabled" : ""}><span>💧</span>TAP TO WATER<small>${ssrInterpolate(unref(finished) ? "FULL BLOOM!" : "KEEP WATERING")}</small></button></div></section>`);
			}
			_push(`<footer><span>GROW TOGETHER. BLOOM FASTER.</span><span>Made for joyful teams 🌱</span></footer></main>`);
		};
	}
});
//#endregion
...
var _sfc_main$1 = {
	...
	setup(__props) {
		...
		const stack = void 0;
		const _Error404 = defineAsyncComponent(() => import('../build/error-404-Cv2p3gKk.mjs'));
