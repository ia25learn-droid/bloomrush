...
export default function Home() {
  ...
  return <main className="shell">
    ...
    {mode === "lobby" ? <section className="lobby">
      ...
      <div className="tap-zone"><div><span>{taps>=99?"🏆":seconds<=0?"⏱":"💧"}</span><h2>{taps>=99?"You bloomed!":seconds<=0?"Time’s up!":"Water your plant!"}</h2><p>{taps>=99?"Goal complete — 99 perfect taps!":seconds<=0?`You made ${taps} of 99 watering taps.`:"Reach 99 taps to grow a full bloom"}</p></div><button className="water-btn" onPointerDown={water} disabled={seconds<=0||taps>=99}><span>💧</span>TAP TO WATER<small>{taps} / 99 TAPS</small></button></div>
    </section>}
